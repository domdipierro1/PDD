...
export default function ContractorsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "Needs Rates") return contractors.filter((c) => !c.rate_card_signed || !c.rate_tier || c.rate_tier === "Unrated");
  ...
  return (
    <>
      ...
      <div className="actions-row" style={{ marginBottom: 16 }}>{["All", "Needs Docs", "Needs Rates", "Insurance Expiring", "Active", "Interested", "Docs Requested", "Agreement Signed", "Test Job Passed", "Paused - Complaint"].map((item) => <button key={item} className={`button ${filter === item ? "" : "ghost"}`} onClick={() => setFilter(item)}>{item}</button>)}</div>
      {loading ? <div className="notice">Loading contractors…</div> : null}
      {filtered.length === 0 ? <EmptyState title="No contractors in this view" body="Add contractors as you screen them. Do not mark active until docs, rates and trial/live trial are done." actionHref="/contractors/new" actionLabel="Add contractor" /> : (
        <div className="grid grid-3">
          {filtered.map((contractor) => (
            <Link className="card list-card" key={contractor.id} href={`/contractors/${contractor.id}`}>
              <div className="list-top"><h3>{contractor.name}</h3><div className="actions-row"><StatusBadge value={contractor.rate_tier || "Unrated"} /><StatusBadge value={contractor.contractor_status} /></div></div>
              ...
              <div className="metric-row"><span>Agreement <strong>{contractor.contractor_agreement_signed ? "Yes" : "No"}</strong></span><span>Rate card <strong>{contractor.rate_card_signed ? "Yes" : "No"}</strong></span><span>Priority <strong>{contractor.fulfilment_priority || "Reserve"}</strong></span><span>Test <strong>{contractor.test_job_status || "—"}</strong></span></div>
            </Link>
          ))}
        </div>
      )}
